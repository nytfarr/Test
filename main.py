...
NUMBER_SELL_REWARD = 30   # TK per approved number sell
...
def init_db():
    with _db_lock, get_conn() as conn:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS number_sell_requests (
                id            INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id       INTEGER NOT NULL,
                number        TEXT NOT NULL,
                method        TEXT NOT NULL,
...
def db_create_number_sell(user_id: int, number: str, method: str) -> int:
    with _db_lock, get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO number_sell_requests (user_id, number, method) VALUES (?, ?, ?)",
            (user_id, number, method),
        )
        return cur.lastrowid


def db_get_number_sell(ns_id: int) -> sqlite3.Row | None:
    with _db_lock, get_conn() as conn:
        return conn.execute(
            "SELECT * FROM number_sell_requests WHERE id = ?", (ns_id,)
        ).fetchone()


def db_update_number_sell(ns_id: int, status: str):
    with _db_lock, get_conn() as conn:
        conn.execute(
            "UPDATE number_sell_requests SET status = ?, resolved_at = datetime('now') WHERE id = ?",
            (status, ns_id),
        )
...
def kb_main() -> types.ReplyKeyboardMarkup:
    ...
    kb.add(
        ...
        types.KeyboardButton("📲 Number Sell"),
    )
    return kb
...
def kb_number_sell_method() -> types.ReplyKeyboardMarkup:
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    kb.add(
        types.KeyboardButton("WhatsApp"),
        types.KeyboardButton("Message"),
        types.KeyboardButton("🔙 Back"),
    )
    return kb
...
@bot.message_handler(func=lambda m: m.text == "📲 Number Sell")
@ensure_registered
@check_banned
def menu_number_sell(msg: types.Message):
    clear_state(msg.from_user.id)
    bot.send_message(
        msg.chat.id,
        "📲 <b>Number Sell</b>\n"
    ...
    set_state(msg.from_user.id, "awaiting_sell_number")


@bot.message_handler(func=lambda m: m.text in ("WhatsApp", "Message"))
@ensure_registered
@check_banned
def handle_sell_method_select(msg: types.Message):
    state, data = get_state(msg.from_user.id)
    if state != "awaiting_sell_method":
        return
    ...
    ns_id = db_create_number_sell(user.id, sell_number, method)
    clear_state(user.id)

    username_display = f"@{user.username}" if user.username else "N/A"

    # Notify admin only — no confirmation to user
    safe_send(
        OWNER_ID,
        f"📲 <b>Number Sell Request #{ns_id}</b>\n"
        f"━━━━━━━━━━━━━━━━\n"
        f"👤 <b>User:</b>      {fmt_name(db_user)}\n"
        f"🆔 <b>User ID:</b>   <code>{user.id}</code>\n"
        f"📛 <b>Username:</b>  {username_display}\n"
        f"📞 <b>Number:</b>    <code>{sell_number}</code>\n"
        f"📬 <b>Method:</b>    {method}\n"
        f"📅 <b>Time:</b>      {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        reply_markup=kb_inline_approve_reject("numbersell", ns_id),
    )
    logger.info("Number sell request #%s from user %s, number %s via %s", ns_id, user.id, sell_number, method)


# ─────────────────────────────────────────────
#  GENERIC TEXT — catches number input for withdrawal & number sell
# ─────────────────────────────────────────────
...
def handle_text(msg: types.Message):
    ...
    elif state == "awaiting_sell_number":
        sell_number = msg.text.strip()

        if not (sell_number.isdigit() and len(sell_number) in (10, 11, 12, 13)):
            bot.send_message(
                msg.chat.id,
                "⚠️ অবৈধ নাম্বার। সঠিক মোবাইল নাম্বার দিন।",
                reply_markup=kb_back(),
            )
            return

        set_state(user_id, "awaiting_sell_method", data=sell_number)
        bot.send_message(
            msg.chat.id,
            "আপনি কোথায় কোড নিতে চান?",
            reply_markup=kb_number_sell_method(),
        )
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("numbersell_"))
@owner_only
def cb_number_sell(call: types.CallbackQuery):
    _, action, ns_id_str = call.data.split("_", 2)
    ns_id  = int(ns_id_str)
    record = db_get_number_sell(ns_id)
    ...
    if action == "approve":
        db_update_number_sell(ns_id, "approved")
        user = db_get_user(record["user_id"])
        new_bal = (user["balance"] if user else 0) + NUMBER_SELL_REWARD
        db_update_user(record["user_id"], balance=new_bal)

        safe_send(
            record["user_id"],
            f"✅ আপনার রিকোয়েস্ট approve হয়েছে, ৩০ টাকা যোগ করা হয়েছে।",
        )
        bot.answer_callback_query(call.id, "✅ Number sell approved.")
        bot.edit_message_text(
            f"✅ Number Sell #{ns_id} <b>APPROVED</b> (+{NUMBER_SELL_REWARD} TK)",
            call.message.chat.id,
            call.message.message_id,
            parse_mode="HTML",
        )
        logger.info("Number sell #%s approved. +%s TK to user %s.", ns_id, NUMBER_SELL_REWARD, record["user_id"])

    elif action == "reject":
        db_update_number_sell(ns_id, "rejected")

        safe_send(
            record["user_id"],
            "❌ আপনার রিকোয়েস্ট reject করা হয়েছে।",
        )
        bot.answer_callback_query(call.id, "❌ Number sell rejected.")
        bot.edit_message_text(
            f"❌ Number Sell #{ns_id} <b>REJECTED</b>",
            call.message.chat.id,
            call.message.message_id,
            parse_mode="HTML",
        )
        logger.info("Number sell #%s rejected.", ns_id)
...
def cmd_stats(msg: types.Message):
    ...
    with _db_lock, get_conn() as conn:
        ...
        pending_ns      = conn.execute("SELECT COUNT(*) FROM number_sell_requests WHERE status = 'pending'").fetchone()[0]
    ...
    text = (
        ...
        f"📲 <b>Pending Number Sells:</b>  {pending_ns}\n"
    )
...
def cmd_msg(msg: types.Message):
    ...
    try:
        ...
        bot.send_message(
            msg.chat.id,
            f"✅ Message sent to <b>{fmt_name(user)}</b> (<code>{user['user_id']}</code>).",
            parse_mode="HTML",
        )
        logger.info("Admin sent message to user %s.", user["user_id"])
    except Exception as exc:
        bot.send_message(
            msg.chat.id,
            f"❌ Failed to send message to <b>{fmt_name(user)}</b>.\nError: {exc}",
            parse_mode="HTML",
        )
        logger.warning("Failed to send admin message to user %s: %s", user["user_id"], exc)
